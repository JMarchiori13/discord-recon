//! Result export (JSON / CSV / JSONL) and formatted console tables.
...
// ---------------------------------------------------------------------------
// JSONL mode (`--stdout`): one JSON object per result row on stdout.
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    fn header_keys_are_sanitized() {
        assert_eq!(header_key("Guild ID"), "guild_id");
        assert_eq!(header_key("Approx. Members"), "approx_members");
        assert_eq!(header_key("UTC Timestamp"), "utc_timestamp");
    }
    ...
    fn row_to_json_includes_module_and_fields() {
        let obj = row_to_json(
            ...
            &["ID", "UTC Timestamp"],
            &[
                "80351110224678912".to_string(),
                "2015-01-01 00:00:00".to_string(),
            ],
        );
        assert_eq!(obj["module"], "snowflake");
        assert_eq!(obj["id"], "80351110224678912");
